...
class Auras:
				...
				for mod in item.get(mod_type, []):
					if m := re.search(r'(.\d+) to Level of Socketed (.*)Gems', mod):
						...
						if not m.group(2):
							...
							descriptor = m.group(2)[:-1].lower()
						level_mods += stats.parse_gem_descriptor(descriptor, value)

					if m := re.search(r'(.\d+)% to Quality of Socketed (.*)Gems', mod):
						value = int(m.group(1))
						if not m.group(2):
							descriptor = ""
						else:
	...
	def support_aura_effect(self, support, level, quality, quality_type, active_skill_info) -> (int, int, int):
		...
		for gem_type in gem_info["support_gem"]["allowed_types"]:
			if any(gem_type.lower() == tag.lower() for tag in active_gem_tags):
				break
		else:
			return 0, 0, 0
